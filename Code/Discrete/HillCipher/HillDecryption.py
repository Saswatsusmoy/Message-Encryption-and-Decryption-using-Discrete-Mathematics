...
def decryption(message):
    with open('Keys/HillCipherKeyMatrix.txt','r') as file:
        a = file.readline()
        b = file.readline()
        c = file.readline()
        d = file.readline()
    ...
    with open("Keys/HillCipherlength.txt",'r') as file:
        length = int((int(file.read())))

    # print(length)

    problist= []
    file = open('Keys/HillCipherProbKeys.txt','r')
    for line in file:
    ...
    file = open("Keys/HillCiphermessage.txt","r") 
    message1 = file.read()
    file.close()
